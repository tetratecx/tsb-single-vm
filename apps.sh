...
if [[ ${ACTION} = "deploy-app-abc" ]]; then
  ...
  # Install app-b on vm and onboard to tsb if needed
  if [[ ${APP_ABC_MODE} = "active-vm" ]] || [[ ${APP_ABC_MODE} = "active-standby-vm" ]]; then
    ...
    # Start app-d in a container (listen on 127.0.0.1)
    if ! ${VM_SSH_EXEC} docker ps | grep containers.dl.tetrate.io/obs-tester-server:1.0 &>/dev/null ; then
      ${VM_SSH_EXEC} docker run -d --restart=always --net=host --name app-b \
          -e SVCNAME=app-b \
    ...
    TSB_VM_ONBOARDING_JWKS=$(cat ${VM_CONFDIR}/sample-jwt-issuer.jwks | awk '{print "                "$0}')
    cat ${ACTIVE_CLUSTER_CONFDIR}/onboarding-vm-patch-template.yaml | sed "s/__TSB_VM_ONBOARDING_JWKS__//g" > ${ACTIVE_CLUSTER_CONFDIR}/onboarding-vm-patch.yaml ;
    echo ${TSB_VM_ONBOARDING_JWKS} >> ${ACTIVE_CLUSTER_CONFDIR}/onboarding-vm-patch.yaml ;
    ...
    # Create WorkloadGroup, Sidecar and OnboardingPolicy for app-b
    ...
    kubectl apply -f ${APP_B_K8S_CONFDIR}/vm ;
    ...
    cat ${VM_CONFDIR}/apps/onboarding.config-app-b-template.yaml | sed s/__TSB_VM_ONBOARDING_ENDPOINT__/${VM_GW_IP}/g > ${VM_CONFDIR}/apps/onboarding.config.yaml ;
