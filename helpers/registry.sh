...
function get_local_registry_endpoint {
  ...
  local registry_ip; registry_ip=$(docker container inspect --format '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' "${container_name}" 2>/dev/null | awk NF) ;
...
function add_insecure_registry {
  ...
  local registry_endpoint; registry_endpoint=$(get_local_registry_endpoint "${container_name}") ;
  ...
  elif grep -q "${registry_endpoint}" /etc/docker/daemon.json; then
...
function sync_single_image {
  ...
  local image_without_registry; image_without_registry=$(echo "${image}" | awk -F/ '{print $NF}') ;
  local image_name; image_name=$(echo "${image_without_registry}" | awk -F: '{print $1}') ;
  local image_tag; image_tag=$(echo "${image_without_registry}" | awk -F: '{print $2}') ;
