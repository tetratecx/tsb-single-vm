...
function generate_vm_jwt_tokens {
  ...
  if ! [[ -f "${mp_output_dir}/vm4/sample-jwt-issuer.jwks" ]]; then
    ...
    export SAMPLE_JWT_EXPIRATION="87600h" ;
    "${BASE_DIR}/output/onboarding-agent-sample-jwt-credential-plugin" generate key -o "${mp_output_dir}/vm4/sample-jwt-issuer" ;
  fi
}
...
function deploy() {
  ...
  # shellcheck disable=SC2002
  JWKS_VM4=$(cat "${mp_output_dir}/vm4/sample-jwt-issuer.jwks" | tr '\n' ' ' | tr -d ' ') ; export JWKS_VM4 ;
  ...
  kubectl --context mgmt apply -f "${SCENARIO_DIR}/k8s/mgmt/03-workload-group.yaml" ;
  kubectl --context mgmt apply -f "${SCENARIO_DIR}/k8s/mgmt/04-service.yaml" ;
  kubectl --context mgmt apply -f "${SCENARIO_DIR}/k8s/mgmt/05-onboarding-policy.yaml" ;
  kubectl --context mgmt apply -f "${SCENARIO_DIR}/k8s/mgmt/06-sidecar.yaml" ;
  kubectl --context mgmt apply -f "${SCENARIO_DIR}/k8s/mgmt/07-ingress-gateway.yaml" ;
  ...
  # shellcheck disable=SC2002
  JWK_VM4=$(cat "${mp_output_dir}/vm4/sample-jwt-issuer.jwk" | tr '\n' ' ' | tr -d ' ') ; export JWK_VM4 ;
  ...
  envsubst < "${SCENARIO_DIR}/vm/vm4/onboard-vm-template.sh" > "${mp_output_dir}/vm4/onboard-vm.sh" ;
...
function undeploy() {
  ...
  offboard_vm "vm4" "${SCENARIO_DIR}/vm/offboard-vm.sh" ;
}
