...
function info() {
  ...
  print_command "kubectl exec -n ns-b -it \"\$(kubectl get pods -l app=client -o jsonpath='{.items[0].metadata.name}' -n ns-b)\" -- fortio load  -t 0 -H \"X-B3-Sampled:1\" svc-b:8080" ;
  echo "Traffic for AppC: daisy-chain-single" ;
  print_command "kubectl exec -n ns-c -it \"\$(kubectl get pods -l app=client -o jsonpath='{.items[0].metadata.name}' -n ns-c)\" -- fortio load  -t 0 -H \"X-B3-Sampled:1\" svc-c:8080" ;
