...
function info() {
  ...
  print_command "kubectl exec -n ns-a -it \"\$(kubectl get pods -l app=client -o jsonpath='{.items[0].metadata.name}' -n ns-a)\" -- fortio load  -t 0 -H \"X-B3-Sampled:1\" http://svc-a:8080" ;
  echo "Traffic for AppB: spray-out-serial" ;
  print_command "kubectl exec -n ns-b -it \"\$(kubectl get pods -l app=client -o jsonpath='{.items[0].metadata.name}' -n ns-b)\" -- fortio load  -t 0 -H \"X-B3-Sampled:1\" http://svc-b:8080" ;
  echo "Traffic for AppC: daisy-chain-single" ;
  print_command "kubectl exec -n ns-c -it \"\$(kubectl get pods -l app=client -o jsonpath='{.items[0].metadata.name}' -n ns-c)\" -- fortio load  -t 0 -H \"X-B3-Sampled:1\" http://svc-c:8080" ;
  echo "Traffic for AppD: daisy-chain-tree" ;
  print_command "kubectl exec -n ns-d -it \"\$(kubectl get pods -l app=client -o jsonpath='{.items[0].metadata.name}' -n ns-d)\" -- fortio load  -t 0 -H \"X-B3-Sampled:1\" http://svc-d:8080" ;
  echo ;
  echo "All dt once in a loop" ;
  print_command "while true; do
  for i in a b c d; do
    kubectl exec -n \"ns-\${i}\" -it \"\$(kubectl get pods -l app=client -o jsonpath='{.items[0].metadata.name}' -n ns-\${i})\" -- fortio load  -t 5s -H \"X-B3-Sampled:1\" \"http://svc-\${i}:8080\" ;
  done
  sleep 1 ;
done" ;
  echo ;
